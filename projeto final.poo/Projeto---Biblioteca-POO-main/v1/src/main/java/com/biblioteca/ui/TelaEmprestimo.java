...
public class TelaEmprestimo extends TelaBaseCadastro {
    ...
    public TelaEmprestimo() {
        ...
        JPanel fundo = criarFundoCadastro("Gerenciar Empréstimos");
        ...
        painelFormulario.add(new JLabel("Data Saída:"));
        txtDataSaida = criarCampoMascara("##/##/####");
        painelFormulario.add(txtDataSaida);

        painelFormulario.add(new JLabel("Data Devolução:"));
        txtDataDevolucao = criarCampoMascara("##/##/####");

        LocalDate hoje = LocalDate.now();
        DateTimeFormatter formato = DateTimeFormatter.ofPattern("dd/MM/yyyy");

        txtDataSaida.setText(hoje.format(formato));
        txtDataDevolucao.setText(hoje.plusDays(15).format(formato));
        ...
        painelFormulario.add(new JLabel("Cópia:"));
        ...
        modeloTabela.addColumn("Data Saída");
    ...
    private void carregarCopias() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this,
                    "Erro ao carregar cópias: " + erro.getMessage());
        }
    }
    ...
    private void carregarFuncionarios() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this,
                    "Erro ao carregar funcionários: " + erro.getMessage());
        }
    }
    ...
    private void salvarEmprestimo() {
        ...
        try {
            if (comboCopia.getSelectedItem() == null) {
                JOptionPane.showMessageDialog(this, "Selecione uma cópia.");
                return;
            }
            ...
            emprestimo.setStatus(comboStatus.getSelectedItem().toString());
            emprestimo.setCopia((Copia) comboCopia.getSelectedItem());
            emprestimo.setLeitor((Leitor) comboLeitor.getSelectedItem());
            emprestimo.setFuncionario((Funcionario) comboFuncionario.getSelectedItem());
    ...
    private void atualizarEmprestimo() {
        ...
        try {
            ...
            emprestimo.setId(Integer.parseInt(modeloTabela.getValueAt(linha, 0).toString()));
            emprestimo.setDataSaida(txtDataSaida.getText());
            emprestimo.setDataDevolucao(txtDataDevolucao.getText());
            emprestimo.setStatus(comboStatus.getSelectedItem().toString());
            emprestimo.setCopia((Copia) comboCopia.getSelectedItem());
            ...
            JOptionPane.showMessageDialog(this, "Empréstimo atualizado!");
        ...
        try {
            ...
            for (Emprestimo emp : lista) {
                ...
                modeloTabela.addRow(new Object[]{
                        ...
                        emp.getStatus(),
                        emp.getCopia().getCodigoPatrimonio(),
                        emp.getLeitor().getNome(),
                        emp.getFuncionario().getNome()
                });
            }
    ...
    private void preencherCamposTabela() {
        ...
        if (linha >= 0) {
            ...
            txtDataSaida.setText(modeloTabela.getValueAt(linha, 1).toString());
            txtDataDevolucao.setText(modeloTabela.getValueAt(linha, 2).toString());
            comboStatus.setSelectedItem(modeloTabela.getValueAt(linha, 3).toString());

            selecionarCopiaPorCodigo(modeloTabela.getValueAt(linha, 4).toString());
            selecionarLeitorPorNome(modeloTabela.getValueAt(linha, 5).toString());
            selecionarFuncionarioPorNome(modeloTabela.getValueAt(linha, 6).toString());
        }
    }

    private void selecionarCopiaPorCodigo(String codigo) {

        for (int i = 0; i < comboCopia.getItemCount(); i++) {
            Copia copia = comboCopia.getItemAt(i);

            if (copia.getCodigoPatrimonio().equals(codigo)) {
                comboCopia.setSelectedIndex(i);
                return;
            }
        }
    }

    private void selecionarLeitorPorNome(String nome) {
    ...
    private void selecionarFuncionarioPorNome(String nome) {

        for (int i = 0; i < comboFuncionario.getItemCount(); i++) {
            Funcionario funcionario = comboFuncionario.getItemAt(i);
